...
def build_g_unet(gen_conf, train_conf) :
    ...
    activation = train_conf['GAN']['generator']['activation']
    patch_shape = train_conf['GAN']['generator']['patch_shape']
    expected_output_shape = train_conf['GAN']['generator']['output_shape']
    spectral_norm = train_conf['GAN']['generator']['spectral_norm']

    if train_conf['GAN']['generator']['model_name'] == []:
        model_name = 'generator_u_net_v2'
    else:
        model_name = train_conf['GAN']['generator']['model_name']
    ...
    #     model_name, dimension, num_classes, input_shape, output_shape, activation, kernel_init, concat_axis, downsize_factor = 2)

    model = __generate_unet_v2_model(model_name, dimension, num_classes, input_shape, output_shape, activation,
                                     spectral_norm, batch_norm, kernel_init, concat_axis, padding_opt="same")
...
# using strides instead of max pooling (not working)
def __generate_unet_v2_model(model_name, dimension, num_classes, input_shape, output_shape, activation, spectral_norm,
                             batch_norm, kernel_init, concat_axis, padding_opt):

    input = Input(shape=input_shape)

    # encoder model: C64-C128-C256-C512-C512
    e1 = encoder_block(input, 64, dimension, kernel_init, padding_opt, spectral_norm, batch_norm)
    e2 = encoder_block(e1, 128, dimension, kernel_init, padding_opt, spectral_norm, batch_norm)
    #e3 = encoder_block(e2, 256, dimension, kernel_init, padding_opt)
    #e4 = encoder_block(e3, 512, dimension, kernel_init, padding_opt)
    #e5 = encoder_block(e4, 512, dimension, kernel_init, padding_opt)
    # e6 = encoder_block(e5, 512, dimension, kernel_init, padding_opt)
    # e7 = encoder_block(e6, 512, dimension, kernel_init, padding_opt)

    # bottleneck, no batch norm and elu
    kernel_size = (4, 4) if dimension == 2 else (4, 4, 4)
    strides = (2, 2) if dimension == 2 else (2, 2, 2)
    if dimension == 2:
        if spectral_norm:
            b = ConvSN2D(256, kernel_size, strides=strides, padding=padding_opt, kernel_initializer=kernel_init)(e2)
        else:
            b = Conv2D(256, kernel_size, strides=strides, padding=padding_opt, kernel_initializer=kernel_init)(e2)
    else:
        if spectral_norm:
            b = ConvSN3D(256, kernel_size, strides=strides, padding=padding_opt,  kernel_initializer=kernel_init)(e2)
        else:
            b = Conv3D(256, kernel_size, strides=strides, padding=padding_opt, kernel_initializer=kernel_init)(e2)
    b = Activation('elu')(b)

    # decoder model
    # d1 = decoder_block(b, e7, 512, dimension, kernel_init, concat_axis, padding_opt, dropout=False)
    # d2 = decoder_block(d1, e6, 512, dimension, kernel_init, concat_axis, padding_opt, dropout=False)
    # d3 = decoder_block(d2, e5, 512, dimension, kernel_init, concat_axis, padding_opt, dropout=False)
    #d4 = decoder_block(b, e4, 512, dimension, kernel_init, concat_axis, padding_opt, dropout=False)
    #d5 = decoder_block(b, e3, 256, dimension, kernel_init, concat_axis, padding_opt, dropout=False)
    d6 = decoder_block(b, e2, 128, dimension, kernel_init, concat_axis, padding_opt, spectral_norm, batch_norm,
                       dropout=False)
    d7 = decoder_block(d6, e1, 64, dimension, kernel_init, concat_axis, padding_opt, spectral_norm, batch_norm,
                       dropout=False)
    # output
    if dimension == 2:
        if spectral_norm:
            g = ConvSN2DTranspose(num_classes, kernel_size, strides=strides, padding=padding_opt, kernel_initializer=kernel_init)(
                d7)
        else:
            g = Conv2DTranspose(num_classes, kernel_size, strides=strides, padding=padding_opt, kernel_initializer=kernel_init)(
                d7)
    else:
        g = Conv3DTranspose(num_classes, kernel_size, strides=strides, padding=padding_opt, kernel_initializer=kernel_init)(
            d7)

    #g = Concatenate(axis=concat_axis)([g, input])

    # print(e1._keras_shape)
    # print(e2._keras_shape)
    # print(e3._keras_shape)
    # print(d1._keras_shape)
    # print(d2._keras_shape)
    # print(d3._keras_shape)
    #print(g._keras_shape)

    # g = get_conv_fc(dimension, g, num_classes, kernel_init)

    g_reshape = Reshape(output_shape)(g)
    if dimension == 3:
        g_reshape = Permute((2, 1))(g_reshape)
    output = Activation(activation)(g_reshape)
...
def encoder_block(layer_in, n_filters, dimension, kernel_init, padding_opt, spectral_norm, batch_norm):
    # add downsampling layer
    kernel_size = (4, 4) if dimension == 2 else (4, 4, 4)
    strides = (2, 2) if dimension == 2 else (2, 2, 2)
    if dimension == 2:
        if spectral_norm:
            g = ConvSN2D(n_filters, kernel_size, strides=strides, padding=padding_opt, kernel_initializer=kernel_init)(
                layer_in)
        else:
            g = Conv2D(n_filters, kernel_size, strides=strides, padding=padding_opt, kernel_initializer=kernel_init)(layer_in)
    else:
        if spectral_norm:
            g = ConvSN3D(n_filters, kernel_size, strides=strides, padding=padding_opt, kernel_initializer=kernel_init)(layer_in)
        else:
            g = Conv3D(n_filters, kernel_size, strides=strides, padding=padding_opt, kernel_initializer=kernel_init)(
                layer_in)
...
def decoder_block(layer_in, skip_in, n_filters, dimension, kernel_init, concat_axis, padding_opt, spectral_norm, batch_norm, dropout=True):
    kernel_size = (4, 4) if dimension == 2 else (4, 4, 4)
    strides = (2, 2) if dimension == 2 else (2, 2, 2)
    # add upsampling layer
    if dimension == 2:
        if spectral_norm:
            g = ConvSN2DTranspose(n_filters, kernel_size, strides=strides, padding=padding_opt,
                                kernel_initializer=kernel_init)(layer_in)
        else:
            g = Conv2DTranspose(n_filters, kernel_size, strides=strides, padding=padding_opt, kernel_initializer=kernel_init)\
                (layer_in)
    else:
        g = Conv3DTranspose(n_filters, kernel_size, strides=strides, padding=padding_opt, kernel_initializer=kernel_init)\
                (layer_in)
    ...
    g = Concatenate(axis=concat_axis)([g, skip_in])
    # elu activation
    g = Activation('elu')(g)
    return g
...
def __generate_unet_model(
    model_name, dimension, num_classes, input_shape, output_shape, activation, kernel_init, concat_axis, spectral_norm,
        batch_norm, downsize_factor=2):

    input = Input(shape=input_shape)

    conv1 = get_conv_core(dimension, input, int(64/downsize_factor), kernel_init, spectral_norm, batch_norm)
    pool1 = get_max_pooling_layer(dimension, conv1)

    conv2 = get_conv_core(dimension, pool1, int(128/downsize_factor), kernel_init, spectral_norm, batch_norm)
    pool2 = get_max_pooling_layer(dimension, conv2)

    conv3 = get_conv_core(dimension, pool2, int(256/downsize_factor), kernel_init, spectral_norm, batch_norm)
    pool3 = get_max_pooling_layer(dimension, conv3)

    conv4 = get_conv_core(dimension, pool3, int(512/downsize_factor), kernel_init, spectral_norm, batch_norm)

    up5 = get_deconv_layer(dimension, conv4, int(256/downsize_factor), kernel_init, spectral_norm)
    up5 = concatenate([up5, conv3], axis=concat_axis)
    conv5 = get_conv_core(dimension, up5, int(256/downsize_factor), kernel_init, spectral_norm, batch_norm)

    up6 = get_deconv_layer(dimension, conv5, int(128/downsize_factor), kernel_init, spectral_norm)
    up6 = concatenate([up6, conv2], axis=concat_axis)
    conv6 = get_conv_core(dimension, up6, int(128/downsize_factor), kernel_init, spectral_norm, batch_norm)

    up7 = get_deconv_layer(dimension, conv6, int(64/downsize_factor), kernel_init, spectral_norm)
    up7 = concatenate([up7, conv1], axis=concat_axis)
    conv7 = get_conv_core(dimension, up7, int(64/downsize_factor), kernel_init, spectral_norm, batch_norm)

    pred = get_conv_fc(dimension, conv7, num_classes, kernel_init, spectral_norm)

    pred_reshape = Reshape(output_shape)(pred)
    if dimension == 3:
        pred_reshape = Permute((2, 1))(pred_reshape)
    output = Activation(activation)(pred_reshape)
...
def get_conv_core(dimension, input, num_filters, kernel_init, spectral_norm, batch_norm):
    x = None
    kernel_size = (3, 3) if dimension == 2 else (3, 3, 3)
    if dimension == 2 :
        if spectral_norm:
            x = ConvSN2D(num_filters, kernel_size=kernel_size, padding='same', kernel_initializer=kernel_init)(input)
        else:
        ...
        x = Activation('elu')(x)
        if spectral_norm:
            x = ConvSN2D(num_filters, kernel_size=kernel_size, padding='same', kernel_initializer=kernel_init)(x)
        else:
        ...
        if spectral_norm:
            x = ConvSN3D(num_filters, kernel_size=kernel_size, padding='same', kernel_initializer=kernel_init)(input)
        else:
        ...
        x = Activation('elu')(x)
        if spectral_norm:
            x = ConvSN3D(num_filters, kernel_size=kernel_size, padding='same', kernel_initializer=kernel_init)(x)
        else:
...
def get_deconv_layer(dimension, input, num_filters, kernel_init, spectral_norm):
    strides = (2, 2) if dimension == 2 else (2, 2, 2)
...
def get_conv_fc(dimension, input, num_filters, kernel_init, spectral_norm) :
    fc = None
...
def build_g_dilated_densenet(gen_conf, train_conf):
    ...
    if train_conf['GAN']['generator']['model_name'] == []:
        model_name = 'generator_fc_densenet_context'
    else:
        model_name = train_conf['GAN']['generator']['model_name']
    ...
    dilated_db_fp_input = False #True # feature pyramid input in dilated db path
    local_path = False # only db path
    db_fp_input = False #False #True   # feature pyramid input in db path
    ...
    is_dilated_conv = True #True #True
    ...
    if dimension == 2:
        ...
        if multi_output == 1:
            if attention_loss == 1:
                output_shape = [(np.prod(expected_output_shape), num_classes[0]),
                                (np.prod(expected_output_shape), num_classes[1]),
                                (np.prod(expected_output_shape), 2)]
            else:
                output_shape = [(np.prod(expected_output_shape), num_classes[0]),
                                (np.prod(expected_output_shape), num_classes[1])]
        else:
            if attention_loss == 1:
                output_shape = [(np.prod(expected_output_shape), num_classes), (np.prod(expected_output_shape), 2)]
            else:
                output_shape = (np.prod(expected_output_shape), num_classes)
    else:
        input_shape = (num_modality, ) + patch_shape
    ...
    if multi_output == 0:
        if activation not in ['softmax', 'sigmoid', 'tanh', 'linear']:
            raise ValueError('activation must be one of "softmax", "sigmoid", "tanh", or "linear"')
    ...
    model = __generate_dilated_densenet(model_name, dimension, num_classes, multi_output, output_name, attention_loss,
                                      overlap_penalty_loss, lamda, kernel_init, padding, random_seed_num, input_shape, output_shape,
                                      ...
                                      spectral_norm, batch_norm, weight_decay, init_conv_filters, include_top, upsampling_type, activation,
...
def build_fc_dense_contextnet(gen_conf, train_conf):
    ...
    dilated_db_fp_input = False #True # feature pyramid input in dilated db path
    local_path = False # only db path
    db_fp_input = True #False #True   # feature pyramid input in db path

    # dilated denseblock
    low_res_input = False #False # True: avg_pooling of globalsegmen encoder input for thalamus case
    is_dilated_init_conv = False # dilated convolution in initial convolution block
    is_dilated_conv = False #True #True
    ...
    model = __generate_fc_densenet_ms(model_name, dimension, num_classes, multi_output, output_name, attention_loss,
                                      overlap_penalty_loss, lamda, kernel_init, padding, random_seed_num, input_shape, output_shape,
                                      expected_output_shape, input_tensor, nb_dense_block, nb_layers_per_block,
                                      growth_rate, reduction, loss_opt, metric_opt, optimizer, initial_lr, dropout_rate,
                                      spectral_norm, batch_norm, weight_decay, init_conv_filters, include_top,
                                      upsampling_type, activation, early_transition, transition_pooling,
                                      initial_kernel_size, is_aspp, global_path, local_path, dilated_db_fp_input,
                                      db_fp_input, low_res_input, is_dilated_init_conv, is_dilated_conv,
                                      dilation_rate_per_block, attn_gates, attn_gates_org_ver, attn_dilation_rate_lst,
                                      glam, glam_arrange_type, glam_input_short_cut, glam_final_conv, glam_position)
...
def __generate_fc_densenet_ms(model_name, dimension, num_classes, multi_output, output_name, attention_loss, overlap_penalty_loss,
                              lamda, kernel_init, padding, random_seed_num, input_shape=None, output_shape=None,
                              ...
                              metric_opt='acc', optimizer='Adam', initial_lr=0.001, dropout_rate=None, spectral_norm=1,
                              batch_norm=False, weight_decay=1e-4, init_conv_filters=48, include_top=None, upsampling_type='deconv',
    ...
    with K.name_scope('DenseNetFCN'):
        ...
        if global_path and local_path:
            ...
            if low_res_input:
                global_input = get_low_res_layer(dimension, img_input, kernel_init, padding, spectral_norm, weight_decay, concat_axis,
                                                 stride = 2, type='average') # global contextual features
            else:
        ...
        if is_aspp:
            bottleneck_out = ASPP(dimension, local_f, global_f, spectral_norm, batch_norm, weight_decay, nb_filter, kernel_init, padding, random_seed_num,
                                  growth_rate, bottleneck_nb_layers, dropout_rate, block_prefix, global_path, local_path,
                                  low_res_input)
        else:
            # bottleneck block
            bottleneck_out = bottleneck_block(dimension, local_f, global_f, spectral_norm, batch_norm, weight_decay, concat_axis,
                                              bottleneck_nb_layers, nb_filter, kernel_init, padding, random_seed_num,
        ...
        output_temp, c_score_up =  transition_up_dense_conv(dimension, multi_output, bottleneck_out, spectral_norm, batch_norm, weight_decay,
                                                           concat_axis, early_transition, nb_dense_block, nb_layers,
                                                           kernel_init, padding, random_seed_num, batch_norm, growth_rate, upsampling_type,
        ...
        output = organise_output(dimension, kernel_init, spectral_norm, concat_axis, output_temp, c_score_up, output_shape,
...
def build_fc_dense_contextnet_feedback(gen_conf, train_conf):
    ...
    global_path = True # dilated db path or db path
    dilated_db_fp_input = False #False #True # feature pyramid input in dilated db path
    local_path = False # only db path
    db_fp_input = True #False #True   # feature pyramid input in db path
    ...
    low_res_input = False #False # True: avg_pooling of globalsegmen encoder input for thalamus case
    is_dilated_init_conv = False # dilated convolution in initial convolution block
    is_dilated_conv = False #False #True #True
    ...
    g_encoder_ch = init_conv_filters
    for nb_layers in nb_layers_per_block[0:3]:
        g_encoder_ch += growth_rate * nb_layers
        if db_fp_input is True:
            g_encoder_ch *= 2  # pyramid input
    if dilated_db_fp_input is True:
        g_encoder_ch *= 2

    if dimension == 2:
        input_shape = [(patch_shape[0], patch_shape[1], num_modality), (np.divide(patch_shape[0], 8).astype(int),
        ...
        if multi_output == 1:
            ...
            if attention_loss == 1:
                ...
                output_shape = [(np.prod(expected_output_shape), num_classes), (np.divide(patch_shape[0], 8).astype(int),
                                                                                np.divide(patch_shape[1], 8).astype(int),
                                                                                g_encoder_ch)]
    else:
        input_shape = [(num_modality, ) + patch_shape, (g_encoder_ch, ) + np.divide(patch_shape, 8).astype(int)]
        if multi_output == 1:
            if attention_loss == 1:
                output_shape = [(num_classes[0], np.prod(expected_output_shape)), (num_classes[1], np.prod(expected_output_shape)),
                                (2, np.prod(expected_output_shape))]
    ...
    model = __generate_fc_densenet_ms_feedback(model_name, dimension, num_classes, multi_output, output_name, attention_loss,
                                      overlap_penalty_loss, lamda, kernel_init, padding, random_seed_num, input_shape, output_shape,
                                      expected_output_shape, input_tensor, nb_dense_block, nb_layers_per_block,
                                      growth_rate, reduction, loss_opt, metric_opt, optimizer, initial_lr, dropout_rate,
                                      spectral_norm, batch_norm, weight_decay, init_conv_filters, include_top, upsampling_type, activation,
    ...
    return model, g_encoder_ch


def __generate_fc_densenet_ms_feedback(model_name, dimension, num_classes, multi_output, output_name, attention_loss, overlap_penalty_loss,
                                      lamda, kernel_init, padding, random_seed_num, input_shape=None, output_shape=None,
                                      expected_output_shape = None, input_tensor=None, nb_dense_block=3,
                                      nb_layers_per_block=(3, 4, 5, 6), growth_rate=12, reduction=0.0, loss_opt='Dice',
                                      metric_opt='acc', optimizer='Adam', initial_lr=0.001, dropout_rate=None,
                                      spectral_norm=1, batch_norm=False, weight_decay=1e-4, init_conv_filters=48,
                                      ...
                                      is_aspp=False, global_path=True, local_path=True, dilated_db_fp_input=True,
                                      db_fp_input=True, low_res_input=True, is_dilated_init_conv=True,
                                      is_dilated_conv=True, dilation_rate_per_block=(2, 4, 8), attn_gates=True,
                                      attn_gates_org_ver=False, attn_dilation_rate_lst=(8, 4, 2), glam=True,
                                      glam_arrange_type='two_way_sequential', glam_input_short_cut=True, glam_final_conv=False,
                                      glam_position='before_shortcut'):
    ...
    # Raises
        ValueError: in case of invalid argument for `reduction`,
            `nb_dense_block`.
    '''
    with K.name_scope('DenseNetFCN_FB'):
        ...
        if input_tensor is None:
            ...
            if not K.is_keras_tensor(input_tensor):
                img_input = Input(tensor=input_tensor, shape=input_shape[0])
                decoder_input = Input(tensor=input_tensor, shape=input_shape[1])
            else:
                img_input = input_tensor
        ...
        # for updating encoder output (decoder input) feedback loop
        tf_decoder_input = Lambda(lambda x: x)(decoder_input)
        is_decoder_in = K.sum(tf_decoder_input, axis= (0, 1, 2, 3), keepdims=False)
        is_decoder_in = K.cast(is_decoder_in, dtype="int32")
        print(is_decoder_in)
        if is_decoder_in == 0:
            decoder_input = global_f

        #global_f = Add()([global_f, decoder_input])

        encoder_out = Activation('elu')(global_f)
        #encoder_out = Activation('relu')(decoder_input)
        print(encoder_out._keras_shape)

        if is_aspp:
            bottleneck_out = ASPP(dimension, local_f, decoder_input, spectral_norm, batch_norm, weight_decay, nb_filter,
                                  kernel_init, padding, random_seed_num, growth_rate, bottleneck_nb_layers, dropout_rate,
                                  block_prefix, global_path, local_path, low_res_input)
        else:
            # bottleneck block
            bottleneck_out = bottleneck_block(dimension, local_f, decoder_input, spectral_norm, batch_norm, weight_decay,
                                              concat_axis, bottleneck_nb_layers, nb_filter, kernel_init, padding,
                                              random_seed_num, growth_rate, dropout_rate, block_prefix, global_path,
                                              local_path, low_res_input, glam, glam_arrange_type, glam_input_short_cut,
                                              glam_final_conv)
        ...
        output_temp, c_score_up =  transition_up_dense_conv(dimension, multi_output, bottleneck_out, weight_decay,
                                                           concat_axis, early_transition, nb_dense_block, nb_layers,
                                                           kernel_init, padding, random_seed_num, spectral_norm,
                                                           batch_norm, growth_rate, upsampling_type,
                                                           skip_list, dropout_rate, include_top, num_classes,
                                                           global_path, local_path, low_res_input, attn_gates,
                                                           attn_gates_org_ver, attn_dilation_rate_lst, glam,
                                                           glam_arrange_type, glam_input_short_cut, glam_final_conv,
        ...
        output = organise_output(dimension, kernel_init, spectral_norm, concat_axis, output_temp, c_score_up, output_shape[0],
                                 activation, output_name, multi_output, attention_loss)
        ...
        model = Model(inputs=[input, decoder_input], outputs=output, name=model_name)
        model.summary()

        return model


def __generate_dilated_densenet(model_name, dimension, num_classes, multi_output, output_name, attention_loss, overlap_penalty_loss,
                              lamda, kernel_init, padding, random_seed_num, input_shape=None, output_shape=None,
                              expected_output_shape = None, input_tensor=None, nb_dense_block=3,
                              nb_layers_per_block=(3, 4, 5, 6), growth_rate=12, reduction=0.0, loss_opt='Dice',
                              metric_opt='acc', optimizer='Adam', initial_lr=0.001, dropout_rate=None,
                              spectral_norm=1, batch_norm=False, weight_decay=1e-4, init_conv_filters=48, include_top=None,
                              upsampling_type='deconv', activation='softmax', early_transition=False, transition_pooling='max',
                              initial_kernel_size=(3, 3, 3), is_aspp=False, global_path=True, local_path=True,
                              dilated_db_fp_input=True, db_fp_input=True, low_res_input=True, is_dilated_init_conv=True,
                              is_dilated_conv=True, dilation_rate_per_block=(2, 4, 8), attn_gates=True,
                              attn_gates_org_ver=False, attn_dilation_rate_lst=(8, 4, 2), glam=True,
    ...
    with K.name_scope('DilatedDenseNet'):
        ...
        # local feature encoder
        if local_path:
            local_f, nb_filter, block_prefix, skip_list_local = conv_dense_transition_down_2(dimension,
                                                                                           local_input,
                                                                                           init_conv_filters,
                                                                                           initial_kernel_size,
                                                                                           kernel_init,
        ...
        # global feature encoder
        if global_path:
            global_f, nb_filter, block_prefix, skip_list_global = conv_dense_transition_down_2(dimension,
                                                                                              global_input,
                                                                                              init_conv_filters,
                                                                                              initial_kernel_size,
                                                                                              kernel_init,
        ...
        # if is_aspp:
        #     bottleneck_out = ASPP(dimension, local_f, global_f, weight_decay, nb_filter, kernel_init, padding, random_seed_num,
        #                           growth_rate, bottleneck_nb_layers, dropout_rate, block_prefix, global_path, local_path,
        #                           low_res_input)
        # else:
        #     # bottleneck block
        ...
        # output_temp, c_score_up = transition_up_dense_conv(dimension, multi_output, bottleneck_out, weight_decay,
        #                                                    concat_axis, early_transition, nb_dense_block, nb_layers,
        #                                                    kernel_init, padding, random_seed_num, growth_rate, upsampling_type,
        #                                                    skip_list, dropout_rate, include_top, num_classes,
        #                                                    global_path, local_path, low_res_input, attn_gates,
        #                                                    attn_gates_org_ver, attn_dilation_rate_lst, glam,
        #                                                    glam_arrange_type, glam_input_short_cut, glam_final_conv,
        ...
        concat = concatenate(skip_list_global, axis=concat_axis)

        channel_axis = 1 if K.image_data_format() == "channels_first" else -1
        #filters = global_f._keras_shape[channel_axis]
        filters = concat._keras_shape[channel_axis]
        ...
        if dimension == 2:
            if spectral_norm:
                ...
                x = ConvSN2D(int(filters / 4), (1, 1), activation='linear', padding=padding, kernel_initializer=kernel_init,
                           use_bias=False)(x)
                output = ConvSN2D(1, (1, 1), activation='linear', padding=padding, kernel_initializer=kernel_init,
                                use_bias=False)(x)
            else:
                x = Conv2D(int(filters / 2), (1, 1), activation='linear', padding=padding,
                           kernel_initializer=kernel_init,
                           use_bias=False)(concat)
                x = Conv2D(int(filters / 4), (1, 1), activation='linear', padding=padding,
                           kernel_initializer=kernel_init,
                           use_bias=False)(x)
                output = Conv2D(1, (1, 1), activation='linear', padding=padding, kernel_initializer=kernel_init,
                                use_bias=False)(x)
        else:
            if spectral_norm:
                ...
                x = ConvSN3D(int(filters / 4), (1, 1, 1), activation='linear', padding=padding,
                           kernel_initializer=kernel_init,
                           use_bias=False)(x)
                output = ConvSN3D(1, (1, 1, 1), activation='linear', padding=padding, kernel_initializer=kernel_init,
                                use_bias=False)(x)
            else:
                x = Conv3D(int(filters / 2), (1, 1, 1), activation='linear', padding=padding,
                           kernel_initializer=kernel_init,
                           use_bias=False)(concat)
                x = Conv3D(int(filters / 4), (1, 1, 1), activation='linear', padding=padding,
                           kernel_initializer=kernel_init,
                           use_bias=False)(x)
                output = Conv3D(1, (1, 1, 1), activation='linear', padding=padding, kernel_initializer=kernel_init,
                                use_bias=False)(x)


        output = organise_output(dimension, kernel_init, spectral_norm, concat_axis, output, None, output_shape,
                                 activation, output_name, multi_output, attention_loss)
...
def organise_output(dimension, kernel_init, spectral_norm, concat_axis, input, c_score_up, output_shape, activation, output_name,
    ...
    if attention_loss == 1:
        ...
        if dimension == 2:
            if spectral_norm:
                c_concate_fc = ConvSN2D(2, (1, 1), padding='same', kernel_initializer=kernel_init)(c_concate)
            else:
                c_concate_fc = Conv2D(2, (1, 1), padding='same', kernel_initializer=kernel_init)(c_concate)
        elif dimension == 3:
            if spectral_norm:
                c_concate_fc = ConvSN3D(2, (1, 1, 1), padding='same', kernel_initializer=kernel_init)(c_concate)
            else:
...
def conv_dense_transition_down(dimension, img_input, init_conv_filters, initial_kernel_size, kernel_init, padding,
                               random_seed_num, spectral_norm, batch_norm, weight_decay, concat_axis, early_transition, compression,
    ...
    if early_transition:
        if not is_dilated_conv:
            x = __transition_down_block(dimension, x, nb_filter, kernel_init, padding, random_seed_num, spectral_norm,
                                        batch_norm, compression=compression,
                                        weight_decay=weight_decay, dropout_rate=dropout_rate,
                                        block_prefix='tr_early_%i' % block_prefix_num,
                                        transition_pooling=transition_pooling, stride=2)
    ...
    for block_idx in range(nb_dense_block):
        ...
        if is_dilated_conv:
            ...
            x, nb_filter, concat_list = __dilated_dense_block(dimension, x, kernel_size, kernel_init, padding, random_seed_num,
                                                              nb_layers[block_idx], dilation_rate_lst, nb_filter, spectral_norm,
                                                              batch_norm, growth_rate, dropout_rate=dropout_rate,
            ...
            x, nb_filter, concat_list = __dense_block(dimension, x, kernel_size, kernel_init, padding, random_seed_num,
                                                      nb_layers[block_idx], dilation_rate, nb_filter, spectral_norm,
                                                      batch_norm, growth_rate, dropout_rate=dropout_rate,
                                                      weight_decay=weight_decay, return_concat_list=True,
        ...
        if is_dilated_conv:
            ...
            #if dimension == 2:
            x_pool = get_low_res_layer(dimension, x, kernel_init, padding, spectral_norm, weight_decay, concat_axis, stride=p, type='')
            ...
            if dilated_db_fp_input and block_idx != 0:
                ...
                img_input, img_input_activation = multi_scale_input(dimension, img_input, ch_x_pool, initial_kernel_size,
                                                                    kernel_init, padding, block_idx, block_prefix_num,
                                                                    spectral_norm, batch_norm, weight_decay, concat_axis)
            ...
            if block_idx == nb_dense_block-1:
                p = pow(2, block_idx + 1)
                #pooling_size = (p, p) if dimension == 2 else (p, p, p)
                x = get_low_res_layer(dimension, x, kernel_init, padding, spectral_norm, weight_decay, concat_axis, stride=p,
                                      type='')
                # if dimension == 2:
                ...
                if dilated_db_fp_input:
                    ...
                    img_input, img_input_activation = multi_scale_input(dimension, img_input, ch_x_pool,
                                                                        initial_kernel_size, kernel_init, padding,
                                                                        block_idx + 1, block_prefix_num, spectral_norm,
                                                                        batch_norm, weight_decay, concat_axis)
            ...
            x = __transition_down_block(dimension, x, nb_filter, kernel_init, padding, random_seed_num, spectral_norm,
                                        batch_norm, compression=compression, weight_decay=weight_decay,
                                        dropout_rate=dropout_rate, block_prefix='tr_%i_%i' % (block_idx, block_prefix_num),
                                        transition_pooling=transition_pooling, stride=2)
...
def conv_dense_transition_down_2(dimension, img_input, init_conv_filters, initial_kernel_size, kernel_init, padding,
                               random_seed_num, spectral_norm, batch_norm, weight_decay, concat_axis, early_transition, compression,
                               transition_pooling, nb_dense_block, nb_layers, growth_rate, dropout_rate,
                               dilated_db_fp_input, db_fp_input, is_dilated_init_conv, is_dilated_conv,
                               dilation_rate_per_block, glam, glam_arrange_type, glam_input_short_cut, glam_final_conv,
    ...
    for block_idx in range(nb_dense_block):
        ...
        if is_dilated_conv:
            ...
            x, nb_filter, concat_list = __dilated_dense_block(dimension, x, kernel_size, kernel_init, padding, random_seed_num,
                                                              ...
                                                              spectral_norm, batch_norm, growth_rate,
        ...
        if is_dilated_conv:
            ...
            # #pooling_size = (p, p) if dimension == 2 else (p, p, p)
            # #if dimension == 2:
            # x_pool = get_low_res_layer(dimension, x, kernel_init, padding, weight_decay, concat_axis, stride=p,
            #                           type='max')
            #
            #     #x_pool = MaxPooling2D(pooling_size)(x)
            # #else:
            ...
            #     p = pow(2, block_idx + 1)
            #     #pooling_size = (p, p) if dimension == 2 else (p, p, p)
            #     x = get_low_res_layer(dimension, x, kernel_init, padding, weight_decay, concat_axis, stride=p,
            #                           type='max')
            #     # if dimension == 2:
            #     #     x = MaxPooling2D(pooling_size)(x)
            #     # else:
...
def bottleneck_block(dimension, local_f, global_f, spectral_norm, batch_norm, weight_decay, concat_axis, bottleneck_nb_layers, nb_filter,
                     kernel_init, padding, random_seed_num, growth_rate, dropout_rate, block_prefix, global_path, local_path,
    ...
    if global_path and local_path:
        db_input = __transition_block(dimension, local_f, global_f, nb_filter, kernel_init, padding, random_seed_num,
                                      low_res_input, spectral_norm, batch_norm, weight_decay=weight_decay, dropout_rate=dropout_rate,
        ...
        if low_res_input:
            db_input = __transition_block(dimension, local_f, global_f, nb_filter, kernel_init, padding, random_seed_num,
                                          low_res_input, spectral_norm, batch_norm, weight_decay=weight_decay,
    ...
    kernel_size = (3, 3) if dimension == 2 else (3, 3, 3)
    dilation_rate = (1, 1) if dimension == 2 else (1, 1, 1)
    _, _, concat_list = __dense_block(dimension, db_input, kernel_size, kernel_init, padding, random_seed_num,
                                      bottleneck_nb_layers, dilation_rate, nb_filter, spectral_norm, batch_norm,
...
def ASPP(dimension, local_f, global_f, spectral_norm, batch_norm, weight_decay, nb_filter, kernel_init, padding,
         random_seed_num, growth_rate, bottleneck_nb_layers, dropout_rate, block_prefix, global_path, local_path,
         low_res_input):
    ...
    with K.name_scope('ASPP'):
        ...
        if dimension == 2:
            if spectral_norm:
                x1 = ConvSN2D(nb_filter, (1, 1), kernel_initializer=kernel_init, padding='same',
                              use_bias=False, dilation_rate=(1, 1), name=name_or_none(block_prefix, '_conv2D_1'))(
                    db_input)
                x2 = ConvSN2D(nb_filter, kernel_size, kernel_initializer=kernel_init, padding='same',
                              use_bias=False, dilation_rate=(2, 2), name=name_or_none(block_prefix, '_conv2D_2'))(
                    db_input)
                x3 = ConvSN2D(nb_filter, kernel_size, kernel_initializer=kernel_init, padding='same',
                              use_bias=False, dilation_rate=(4, 4), name=name_or_none(block_prefix, '_conv2D_3'))(
                    db_input)
            else:
            ...
            if spectral_norm:
                x1 = ConvSN3D(nb_filter, (1, 1, 1), kernel_initializer=kernel_init, padding='same',
                            use_bias=False, dilation_rate=(1, 1, 1), name=name_or_none(block_prefix, '_conv3D_1'))(
                    db_input)
                x2 = ConvSN3D(nb_filter, kernel_size, kernel_initializer=kernel_init, padding='same',
                            use_bias=False, dilation_rate=(2, 2, 2), name=name_or_none(block_prefix, '_conv3D_2'))(
                    db_input)
                x3 = ConvSN3D(nb_filter, kernel_size, kernel_initializer=kernel_init, padding='same',
                            use_bias=False, dilation_rate=(4, 4, 4), name=name_or_none(block_prefix, '_conv3D_3'))(
                    db_input)
            else:
        ...
        if dimension == 2:
            if spectral_norm:
                out = ConvSN2D(out_channel, (1, 1), kernel_initializer=kernel_init, padding='same',
            ...
            if spectral_norm:
                out = ConvSN3D(out_channel, (1, 1, 1), kernel_initializer=kernel_init, padding='same',
                             use_bias=False, dilation_rate=(1, 1, 1), kernel_regularizer=l2(weight_decay),
...
def transition_up_dense_conv(dimension, multi_output, l, weight_decay, concat_axis, early_transition, nb_dense_block,
                             nb_layers, kernel_init, padding, random_seed_num, spectral_norm, batch_norm, growth_rate,
                             upsampling_type, skip_list, dropout_rate, include_top, num_classes, global_path, local_path,
                             low_res_input, attn_gates, attn_gates_org_ver, attn_dilation_rate_lst, glam,
    ...
    for block_idx in range(nb_dense_block):
        ...
        t = __transition_up_block(dimension, l, kernel_init, padding, spectral_norm, nb_filters=n_filters_keep,
        ...
        if global_path and local_path:
            skip_connection = __transition_block(dimension, skip_list[0][block_idx], skip_list[1][block_idx],
                                                 n_filters_keep, kernel_init, padding, random_seed_num, low_res_input,
                                                 spectral_norm, batch_norm, weight_decay=weight_decay,
            ...
            if low_res_input:
                skip_connection = __transition_block(dimension, None, skip_list[1][block_idx], n_filters_keep,
                                                     kernel_init, padding, random_seed_num, low_res_input,
                                                     spectral_norm, batch_norm, weight_decay=weight_decay,
                                                     dropout_rate=dropout_rate, block_prefix='skip_tr_%i' % block_idx)
        ...
        if attn_gates:
            if attn_gates_org_ver:
                ...
                skip_connection, compatibility_score = grid_attention(dimension, skip_connection, l,
                                                                      attn_dilation_rate_lst,
                                                                      nb_dense_block - block_idx, n_filters_keep,
                                                                      kernel_init, weight_decay=weight_decay)
            ...
            if dimension == 2:
                if spectral_norm:
                    skip_connection = ConvSN2D(n_filters_keep, (1, 1), kernel_initializer=kernel_init, padding=padding,
                                             use_bias=False, kernel_regularizer=l2(weight_decay))(skip_connection)
                else:
                    skip_connection = Conv2D(n_filters_keep, (1, 1), kernel_initializer=kernel_init, padding=padding,
                                             use_bias=False, kernel_regularizer=l2(weight_decay))(skip_connection)
            elif dimension == 3:
                if spectral_norm:
                    skip_connection = ConvSN3D(n_filters_keep, (1, 1, 1), kernel_initializer=kernel_init, padding=padding,
                                             use_bias=False, kernel_regularizer=l2(weight_decay))(skip_connection)
                else:
                    skip_connection = Conv3D(n_filters_keep, (1, 1, 1), kernel_initializer=kernel_init, padding=padding,
                                             use_bias=False, kernel_regularizer=l2(weight_decay))(skip_connection)
            else:
                raise NotImplemented
    ...
    if early_transition:
        x_up = __transition_up_block(dimension, x_up, kernel_init, padding, spectral_norm, nb_filters=nb_filter,
                                     type=upsampling_type, weight_decay=weight_decay, block_prefix='tr_up_early', stride=2)
    if include_top:
        if dimension == 2:
            if multi_output == 1:
                if spectral_norm:
                    x1 = ConvSN2D(num_classes[0], (1, 1), activation='linear', padding=padding, use_bias=False,
                                kernel_initializer=kernel_init)(x_up)
                    x2 = ConvSN2D(num_classes[1], (1, 1), activation='linear', padding=padding, use_bias=False,
                                kernel_initializer=kernel_init)(x_up)
                else:
                    x1 = Conv2D(num_classes[0], (1, 1), activation='linear', padding=padding, use_bias=False,
                                kernel_initializer=kernel_init)(x_up)
                    x2 = Conv2D(num_classes[1], (1, 1), activation='linear', padding=padding, use_bias=False,
                                kernel_initializer=kernel_init)(x_up)
                x = [x1, x2]
            else:
                if spectral_norm:
                    x = ConvSN2D(num_classes, (1, 1), activation='linear', padding=padding, use_bias=False,
                               kernel_initializer=kernel_init)(x_up)
                else:
                    x = Conv2D(num_classes, (1, 1), activation='linear', padding=padding, use_bias=False,
                               kernel_initializer=kernel_init)(x_up)
        else:
            if multi_output == 1:
                if spectral_norm:
                    x1 = ConvSN3D(num_classes[0], (1, 1, 1), activation='linear', padding=padding, use_bias=False,
                                kernel_initializer=kernel_init)(x_up)
                    x2 = ConvSN3D(num_classes[1], (1, 1, 1), activation='linear', padding=padding, use_bias=False,
                                kernel_initializer=kernel_init)(x_up)
                else:
                    x1 = Conv3D(num_classes[0], (1, 1, 1), activation='linear', padding=padding, use_bias=False,
                                kernel_initializer=kernel_init)(x_up)
                    x2 = Conv3D(num_classes[1], (1, 1, 1), activation='linear', padding=padding, use_bias=False,
                                kernel_initializer=kernel_init)(x_up)
                x = [x1, x2]
            else:
                if spectral_norm:
                    x = ConvSN3D(num_classes, (1, 1, 1), activation='linear', padding=padding, use_bias=False,
                               kernel_initializer=kernel_init)(x_up)
                else:
                    x = Conv3D(num_classes, (1, 1, 1), activation='linear', padding=padding, use_bias=False,
                               kernel_initializer=kernel_init)(x_up)
        output = x
    else:
        output = x_up
...
def __conv_block(dimension, x, kernel_size, kernel_init, padding, random_seed_num, spectral_norm, batch_norm, nb_filter,
                 bottleneck=False, dropout_rate=None, weight_decay=1e-4, dilation_rate=(1, 1, 1), block_prefix=None):
    '''
...
def __dense_block(dimension, x, kernel_size, kernel_init, padding, random_seed_num, nb_layers, dilation_rate, nb_filter,
                  spectral_norm, batch_norm, growth_rate, bottleneck=False, dropout_rate=None, weight_decay=1e-4,
    ...
    with K.name_scope('DenseBlock'):
        ...
        for i in range(nb_layers):
            cb = __conv_block(dimension, x, kernel_size, kernel_init, padding, random_seed_num, spectral_norm, batch_norm,
                              growth_rate, bottleneck, dropout_rate, weight_decay, dilation_rate,
...
def __dilated_dense_block(dimension, x, kernel_size, kernel_init, padding, random_seed_num, nb_layers, dilation_rate_lst,
                          nb_filter, spectral_norm, batch_norm, growth_rate, bottleneck=False, dropout_rate=None, weight_decay=1e-4,
                          grow_nb_filters=True, return_concat_list=False, block_prefix=None):
    '''
    Build a dense_block where the output of each conv_block is fed
    to subsequent ones
...
def __transition_down_block(dimension, x, nb_filter, kernel_init, padding, random_seed_num, spectral_norm, batch_norm,
                            compression=1.0, weight_decay=1e-4, dropout_rate=None, block_prefix=None,
                            transition_pooling='max', stride=2):
    '''
...
def __transition_up_block(dimension, ip, kernel_init, padding, spectral_norm, nb_filters, type='deconv', weight_decay=1E-4,
    ...
    with K.name_scope('TransitionUp'):
        ...
        if dimension == 2:
            if type == 'upsampling':
                ...
                if spectral_norm:
                    x = ConvSN2D(nb_filters, (3, 3), activation='elu', padding=padding,
                               kernel_regularizer=l2(weight_decay), use_bias=False,
                               kernel_initializer=kernel_init,
                               name=name_or_none(block_prefix, '_conv2D'))(ip)
                else:
                    x = Conv2D(nb_filters, (3, 3), activation='elu', padding=padding,
                               kernel_regularizer=l2(weight_decay), use_bias=False,
                               kernel_initializer=kernel_init,
                               name=name_or_none(block_prefix, '_conv2D'))(ip)
                ...
                if spectral_norm:
                    x = ConvSN2DTranspose(nb_filters, (3, 3), activation='elu', padding=padding,
                                        strides=strides, kernel_initializer=kernel_init,
                                        kernel_regularizer=l2(weight_decay),
                                        name=name_or_none(block_prefix, '_conv2DT'))(ip)
                else:
                    x = Conv2DTranspose(nb_filters, (3, 3), activation='elu', padding=padding,
                                        strides=strides, kernel_initializer=kernel_init,
                                        kernel_regularizer=l2(weight_decay),
                                        name=name_or_none(block_prefix, '_conv2DT'))(ip)
            ...
            if type == 'upsampling':
                ...
                if spectral_norm:
                    x = ConvSN3D(nb_filters, (3, 3, 3), activation='elu', padding=padding,
                               kernel_regularizer=l2(weight_decay), use_bias=False,
                               kernel_initializer=kernel_init,
                               name=name_or_none(block_prefix, '_conv3D'))(ip)
                else:
                    x = Conv3D(nb_filters, (3, 3, 3), activation='elu', padding=padding,
                               kernel_regularizer=l2(weight_decay), use_bias=False,
                               kernel_initializer=kernel_init,
                               name=name_or_none(block_prefix, '_conv3D'))(ip)
                x = SubPixelUpscaling(scale_factor=2,
                ...
                x = Conv3DTranspose(nb_filters, (3, 3, 3), activation='elu', padding=padding,
                                    strides=strides, kernel_initializer=kernel_init,
                                    kernel_regularizer=l2(weight_decay),
...
def __transition_block(dimension, local_f, global_f, nb_filter, kernel_init, padding, random_seed_num, low_res_input,
                       spectral_norm, batch_norm, weight_decay=1e-4, dropout_rate=None, block_prefix=None):
    ...
    with K.name_scope('Transition'):
        ...
        if dimension == 2:
            if local_f is not None:
                if spectral_norm:
                    local_f = ConvSN2D(nb_filter, (1, 1), kernel_initializer=kernel_init,
                               padding=padding, use_bias=False, kernel_regularizer=l2(weight_decay),
                    ...
                    local_f = Conv2D(nb_filter, (1, 1), kernel_initializer=kernel_init,
                               ...
                               name=name_or_none(block_prefix, '_conv2D_normal'))(local_f)
            if spectral_norm:
                global_f = ConvSN2D(nb_filter, (1, 1), kernel_initializer=kernel_init,
                           padding=padding, use_bias=False, kernel_regularizer=l2(weight_decay),
            ...
            if local_f is not None:
                if spectral_norm:
                    local_f = ConvSN3D(nb_filter, (1, 1, 1), kernel_initializer=kernel_init,
                                     padding=padding, use_bias=False, kernel_regularizer=l2(weight_decay),
                    ...
                    local_f = Conv3D(nb_filter, (1, 1, 1), kernel_initializer=kernel_init,
                               ...
                               name=name_or_none(block_prefix, '_conv3D_normal'))(local_f)
            if spectral_norm:
                global_f = ConvSN3D(nb_filter, (1, 1, 1), kernel_initializer=kernel_init,
                                  padding=padding, use_bias=False, kernel_regularizer=l2(weight_decay),
...
def get_low_res_layer(dimension, input, kernel_init, padding, spectral_norm, weight_decay, channel_axis, stride=2, type='max'):
    strides = (stride, stride) if dimension == 2 else (stride, stride, stride)
    if dimension == 2 :
        if spectral_norm:
            input = ConvSN2D(int(input._keras_shape[channel_axis]), (1, 1), kernel_initializer=kernel_init,
                       padding=padding, strides=strides, use_bias=False, kernel_regularizer=l2(weight_decay))(input)
        else:
            input = Conv2D(int(input._keras_shape[channel_axis]), (1, 1), kernel_initializer=kernel_init,
                       padding=padding, strides=strides, use_bias=False, kernel_regularizer=l2(weight_decay))(input)
        if stride == 1:
            if type == 'average':
        ...
        if spectral_norm:
            input = ConvSN3D(int(input._keras_shape[channel_axis]), (1, 1, 1), kernel_initializer=kernel_init,
                       padding=padding, strides=strides, use_bias=False, kernel_regularizer=l2(weight_decay))(input)
        else:
            input = Conv3D(int(input._keras_shape[channel_axis]), (1, 1, 1), kernel_initializer=kernel_init,
                           padding=padding, strides=strides, use_bias=False, kernel_regularizer=l2(weight_decay))(input)
        if stride == 1:
            if type == 'average':
...
def multi_scale_input(dimension, input, nb_filter, initial_kernel_size, kernel_init, padding, block_idx, block_prefix_num,
                      spectral_norm, batch_norm, weight_decay, concat_axis):
    input = get_low_res_layer(dimension, input, kernel_init, padding, spectral_norm, weight_decay, concat_axis,
                                                 stride = 2, type='max')
    if dimension == 2:
        if spectral_norm:
            input_conv = ConvSN2D(nb_filter, initial_kernel_size,
                                kernel_initializer=kernel_init, padding=padding,
        ...
        if spectral_norm:
            input_conv = ConvSN3D(nb_filter, initial_kernel_size,
                                kernel_initializer=kernel_init, padding=padding,
            ...
            input_conv = Conv3D(nb_filter, initial_kernel_size,
                       ...
                       kernel_regularizer=l2(weight_decay))(input)
    if batch_norm:
        input_conv = BatchNormalization(axis=concat_axis, epsilon=1.1e-5,
                                          name='multi_input_bn_%i_%i' % (block_idx, block_prefix_num))(input_conv)
    input_activation = Activation('elu')(input_conv)
