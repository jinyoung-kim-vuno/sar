...
The 3d version was added by Jinyoung Kim
...
class Length_3d(layers.Layer):
    def __init__(self, num_classes, seg=True, **kwargs):
        super(Length_3d, self).__init__(**kwargs)
        if num_classes == 2:
            self.num_classes = 1
        else:
            self.num_classes = num_classes
    ...
    def call(self, inputs, **kwargs):
        assert inputs.get_shape()[-2].value == self.num_classes, \
            'Error: Must have num_capsules = num_class going into Length'  # 'Error: Must have num_capsules = 1 going into Length'
        #if inputs.get_shape().ndims == 6:
            #inputs = K.squeeze(inputs, axis=-2)
        inputs_transposed = tf.transpose(inputs, [0, 1, 2, 3, 5, 4])
        input_norms = tf.norm(inputs_transposed, axis=-2)
        return input_norms
        #return K.expand_dims(tf.norm(inputs, axis=-1), axis=-1)
    ...
    def get_config(self):
        config = {'num_classes': self.num_classes, 'seg': self.seg}
        base_config = super(Length_3d, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
...
class Mask_3d(layers.Layer):
    def __init__(self, resize_masks=False, **kwargs):
        super(Mask_3d, self).__init__(**kwargs)
        self.resize_masks = resize_masks
    ...
    def call(self, inputs, **kwargs):
        if type(inputs) is list:
            assert len(inputs) == 2
            input, mask = inputs
            _, hei, wid, axi, _, _ = input.get_shape()
            if self.resize_masks:
                mask = tf.image.resize_bicubic(mask, (hei.value, wid.value, axi.value))
            mask = K.expand_dims(mask, -1)
            if input.get_shape().ndims == 3:
                masked = K.batch_flatten(mask * input)
            else:
    ...
    def get_config(self):
        config = {'resize_masks': self.resize_masks}
        base_config = super(Mask_3d, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
...
class ConvCapsuleLayer3D(layers.Layer):
    ...
    def build(self, input_shape):
        assert len(input_shape) == 6, "The input Tensor should have shape=[None, input_height, input_width, " \
                                      "input_axis, input_num_capsule, input_num_atoms]"
        self.input_height = input_shape[1]
        self.input_width = input_shape[2]
        self.input_axis = input_shape[3]
    ...
    def call(self, input_tensor, training=None):
        ...
        input_shape = K.shape(input_transposed)
        input_tensor_reshaped = K.reshape(input_transposed, [
            input_shape[0] * input_shape[1], self.input_height, self.input_width, self.input_axis, self.input_num_atoms])
        input_tensor_reshaped.set_shape((None, self.input_height, self.input_width, self.input_axis, self.input_num_atoms))

        conv = K.conv3d(input_tensor_reshaped, self.W, (self.strides, self.strides, self.strides),
                        padding=self.padding, data_format='channels_last')

        votes_shape = K.shape(conv)
        _, conv_height, conv_width, conv_axis, _ = conv.get_shape()

        votes = K.reshape(conv, [input_shape[1], input_shape[0], votes_shape[1], votes_shape[2], votes_shape[3],
                                 self.num_capsule, self.num_atoms])
        votes.set_shape((None, self.input_num_capsule, conv_height.value, conv_width.value, conv_axis.value,
                         self.num_capsule, self.num_atoms))

        logit_shape = K.stack([
            input_shape[1], input_shape[0], votes_shape[1], votes_shape[2], votes_shape[3], self.num_capsule])
        biases_replicated = K.tile(self.b, [conv_height.value, conv_width.value, conv_axis.value, 1, 1])

        activations = update_routing_3d(
            votes=votes,
            biases=biases_replicated,
            logit_shape=logit_shape,
            num_dims=7,
...
class DeconvCapsuleLayer3D(layers.Layer):
    def __init__(self, kernel_size, num_capsule, num_atoms, scaling=2, upsamp_type='deconv', padding='same', routings=3,
                 kernel_initializer='he_normal', **kwargs):
        super(DeconvCapsuleLayer3D, self).__init__(**kwargs)
        self.kernel_size = kernel_size
        self.num_capsule = num_capsule
        self.num_atoms = num_atoms
        self.scaling = scaling
    ...
    def call(self, input_tensor, training=None):
        ...
        input_shape = K.shape(input_transposed)
        input_tensor_reshaped = K.reshape(input_transposed, [
            input_shape[1] * input_shape[0], self.input_height, self.input_width, self.input_axis, self.input_num_atoms])
        input_tensor_reshaped.set_shape((None, self.input_height, self.input_width, self.input_axis, self.input_num_atoms))

        if self.upsamp_type == 'resize':
            upsamp = K.resize_volumes(input_tensor_reshaped, self.scaling, self.scaling, self.scaling, 'channels_last')
            ...
            out_height = deconv_length(self.input_height, self.scaling, self.kernel_size, self.padding, None)
            out_width = deconv_length(self.input_width, self.scaling, self.kernel_size, self.padding, None)
            out_axis = deconv_length(self.input_axis, self.scaling, self.kernel_size, self.padding, None)
            output_shape = (batch_size, out_height, out_width, out_axis, self.num_capsule * self.num_atoms)

            outputs = K.conv3d_transpose(input_tensor_reshaped, self.W, output_shape,
                                         (self.scaling, self.scaling, self.scaling), padding=self.padding,
                                         data_format='channels_last')

        votes_shape = K.shape(outputs)
        _, conv_height, conv_width, conv_axis, _ = outputs.get_shape()

        votes = K.reshape(outputs, [input_shape[1], input_shape[0], votes_shape[1], votes_shape[2], votes_shape[3],
                                 self.num_capsule, self.num_atoms])
        votes.set_shape((None, self.input_num_capsule, conv_height.value, conv_width.value, conv_axis.value,
                         self.num_capsule, self.num_atoms))
    ...
    def compute_output_shape(self, input_shape):
        ...
        output_shape[1] = deconv_length(output_shape[1], self.scaling, self.kernel_size, self.padding, None)
        output_shape[2] = deconv_length(output_shape[2], self.scaling, self.kernel_size, self.padding, None)
        output_shape[3] = deconv_length(output_shape[3], self.scaling, self.kernel_size, self.padding, None)
        output_shape[4] = self.num_capsule
        output_shape[5] = self.num_atoms
    ...
    def get_config(self):
        ...
        base_config = super(DeconvCapsuleLayer3D, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
...
def update_routing_3d(votes, biases, logit_shape, num_dims, input_dim, output_dim,
                    num_routing):
    if num_dims == 7:
    ...
    votes_trans = tf.transpose(votes, votes_t_shape)
    _, _, _, height, width, axis, caps = votes_trans.get_shape()
